...
class Struct:
    ...
    exactly_one: tuple[str, ...] | None = None
...
def struct_of(name: str, node: dict[str, Any]) -> Struct:
    ...
    if check is not None:
        ...
        elif kind == "exactly-one":
            members = tuple(str(m) for m in check["members"])
            declared = {f.name for f in struct.fields}
            optional = {f.name for f in struct.fields if not f.required}
            if not set(members) <= optional or set(members) != declared:
                raise SchemaError(f"{name}: exactly-one must name every member, all optional")
            if node.get("minProperties") != 1 or node.get("maxProperties") != 1:
                raise SchemaError(f"{name}: exactly-one must state min/maxProperties 1")
            struct.exactly_one = members
        else:
            raise SchemaError(f"{name}: unknown x-direwolf-check {check}")
    if ("minProperties" in node or "maxProperties" in node) and struct.exactly_one is None:
        raise SchemaError(f"{name}: min/maxProperties are only understood with exactly-one")
    return struct
...
def render_struct(struct: Struct) -> Iterator[str]:
    ...
    if struct.exactly_one is not None:
        yield "        validate.exactly_one("
        yield "            ("
        for member in struct.exactly_one:
            yield f"                ({member!r}, {member}_ is not None),"
        yield "            ),"
        yield "            cx,"
        yield "        )"
...
def generate_family(family: str, summary: str) -> tuple[str, list[Path]]:
    ...
    specs: dict[tuple[str, str], list[tuple[int, str]]] = {}
    for path in files:
        doc = load(path)
        ...
        low, high = int(versions["min"]), int(versions["max"])
        # One schema file per version; a spec spanning several versions is
        # written by each of its files, identically, and kept once.
        spec = (
            f"        MessageSpec(\n"
            f"            schema={name!r},\n"
            f"            message_type={mtype!r},\n"
            f"            versions=VersionRange({low}, {high}),\n"
            f"            rules=Rules({rules}),\n"
            f"            payload={doc['x-direwolf-payload']},\n"
            f"        ),"
        )
        entries = specs.setdefault((mtype, name), [])
        if (low, spec) not in entries:
            entries.append((low, spec))
    ...
    lines += [
        ...
        *[
            line
            for (mtype, name), entries in sorted(specs.items())
            for line in (
                f"    ({mtype!r}, {name!r}): (",
                *[spec for _, spec in sorted(entries)],
                "    ),",
            )
        ],
        "})",
