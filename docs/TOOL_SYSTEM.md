...
- **`capabilities`** may be a function of arguments — `fs.read` needs `fs.read:<the actual path>`, computed by the canonicaliser, not by the tool. As implemented for `fs.read` at M4b: `fs.read:<canonical path>?max_bytes=<the requested bound>&no_symlink_targets=true`, derived by the authority; the request states neither ([ADR-0043](adr/0043-m4b-private-broker-channel-and-brokered-fs-read.md)). As implemented for the other filesystem tools at M4c, a call's capabilities are a **plan** of canonical actions, each decided by both gates, all of which must allow: `fs.search` needs `fs.read` bounded by its scan; `fs.write` needs `fs.write`, and `fs.create` too when the target is vacant; `fs.patch` needs `fs.read` bounded by its base and `fs.write` bounded by its post revision; `fs.move` needs `fs.delete` on its source and `fs.create` on its destination ([ADR-0044](adr/0044-m4c-filesystem-operations-plans-and-atomic-mutation.md) §4). The eight filesystem tools below have wire forms (version 2 of the tool messages). As implemented for the process tools at M4d ([ADR-0045](adr/0045-m4d-process-execution-broker.md) §§6, 7, 14): `process.exec` needs `process.exec` on the **executable identity** — the canonical path the authority resolved an absolute host path to, and the SHA-256 of that file — bound to the first argument by `argv_allowlist`; `process.status` needs `process.inspect` and `process.kill` `process.signal` on the identity the process was launched as; `max_output_bytes=N` is the combined stdout+stderr bound (each stream keeps its first ⌊N/2⌋ bytes, at most 128 KiB). The eleven filesystem and process tools have wire forms (version 3 of the tool messages; no production build launches a process before M6); the rest of this inventory is design.
...
| 9 | `process.exec` | varies; declared per invocation | `NON_RETRYABLE` (as built at M4d; an unproved launch is `UNKNOWN` and never repeated) |
