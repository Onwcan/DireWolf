...
- *Filesystem Broker* — file operations on descriptors authority checked and handed over. **M4b implements one:** `fs.read` of one regular file, received as exactly one read-only descriptor over `SCM_RIGHTS`, re-proven by `(st_dev, st_ino)`, mode and kind, and read with `pread` up to the authorised bound and never a byte past it; the broker never opens a path ([ADR-0043](adr/0043-m4b-private-broker-channel-and-brokered-fs-read.md)). The pinned root is never sent. **M4c adds the rest** ([ADR-0044](adr/0044-m4c-filesystem-operations-plans-and-atomic-mutation.md)): `fs.stat` on an `O_PATH` descriptor; `fs.list` through a directory descriptor, sorted, never recursive; `fs.search` for a literal byte string in a bounded scan, offsets only; and `fs.write`, `fs.patch`, `fs.move` and `fs.delete` on one validated name in a parent directory the authority opened — a new file written and synced in a private staging directory, then `renameat2(RENAME_EXCHANGE)` or `RENAME_NOREPLACE`, both directories synced; a move one no-replace rename; a delete staged, proved, then removed. Every change checks by `(st_dev, st_ino)` that the name binds the authorised object immediately before the system call and again after it, and undoes — provably, or else `UNKNOWN` — a change that reached anything else; Linux has no compare-and-swap of a name against an inode, so untrusted concurrent writers are excluded by the permission model rather than by the kernel. Changing names needs the broker's own uid to have directory write permission — ambient authority an operator grants on a write-enabled workspace. Every staging directory is recorded with its invocation's intent and reclaimed only when it provably holds the broker's own uncommitted data, beneath the root re-pinned by its fingerprint. After every namespace change the broker `fsync`s every directory it changed before the next.
- *Exec Broker* — spawns processes with a verified executable, scrubbed environment, resource limits. **M4d implements it** ([ADR-0045](adr/0045-m4d-process-execution-broker.md)): the broker receives the executable and the working directory as two read-only descriptors, re-proves their identities and the executable's trust attributes and SHA-256 through the descriptor it will execute, and a launch helper (its own binary, empty environment, stdin `/dev/null`, its own process group, resource limits, `no_new_privs`, no surviving descriptor) executes that descriptor with `execveat(AT_EMPTY_PATH)` — no path, no shell, no `/proc/self/fd`. It drains both streams to a bound, supervises by pidfd, kills by pidfd and process group, and holds at most 8 processes per generation. No production authority issues a launch before M6's approvals: every process would run on the host with the broker's privileges.
