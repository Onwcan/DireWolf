...
**Acceptance:** round-trip property tests in both languages; **DWKP rejects unknown fields and unknown operations** (and duplicate keys, and NFC-colliding keys); **DWCP preserves unknown fields**; **the event log retains unknown events verbatim** — three separate tests, per [ADR-0023](adr/0023-dwkp-strict-schema.md), not one blanket assertion; schema-drift CI gate; parser fuzzed ≥ 1 h clean.
