...
**Decomposed** into M3a (architecture decisions and wire forms — [ADR-0035](adr/0035-m3-authority-dependency-set.md), [ADR-0036](adr/0036-m3-authority-operations-and-the-capability-wire-form.md); `AdmitRun` carries a mandatory idempotency key, a wire decision is `ALLOW` or `DENY` until M6 can honour a third, and an authority-state refusal is a typed message distinct from both a protocol error and a policy denial), M3b (capabilities and attenuation), M3c (the policy engine), M3d (`kernel.db`, epochs, leases, admission, audit) and M3e (the real UDS authority server, peer credentials and hostile real-process evals). Each stage is verifiable on its own; the acceptance criteria below are the milestone's, not any one stage's.
