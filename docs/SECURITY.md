...
10. **As of M3c, the two decision functions exist and nothing calls them.** A DWKP message that decodes is well-formed and nothing more. The capability lattice (M3b) can answer "is this within the authority held?" and the policy engine (M3c) can answer "should this be allowed?", both deterministically and both tested — but there is no kernel process serving requests, no transport, no `kernel.db`, no audit log, no approval registry and no canonicaliser, so **no request is authorised, no policy is enforced at any real boundary, no capability is validated against a real grant, no secret is protected and there is no process isolation** (M3d–M5). A decision function nothing calls decides nothing.
