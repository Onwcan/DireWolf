...
10. **As of M3d, the authority has durable state; as of M3e it has a process boundary** (see item 11). A DWKP message that decodes is well-formed and nothing more. The capability lattice (M3b) answers "is this within the authority held?", the policy engine (M3c) answers "should this be allowed?", and M3d gives both a durable subject: `kernel.db` holds fenced epochs, leases, admissions, grants and every policy input, and `audit.log` holds a hash chain written before any answer is returned ([ADR-0039](adr/0039-durable-authority-state.md)). There is no approval registry, and the canonicaliser (M4a, [ADR-0042](adr/0042-m4a-canonical-filesystem-resolution.md)) resolves what a filesystem path names beneath an operator-bound root but nothing acts on the result, so **no effect is authorised, no secret is protected and there is no sandbox** (M4b–M6). Policy inputs whose real producers do not exist yet are derived in the restrictive direction — every run is unattended (`api`), taint starts at none and can only rise — and are not end-to-end proven. And no proposed action is decided over DWKP before M4b: a resolved path is one fact of an action, and without the rest the kernel cannot know where an action would run or what it would reach, so `QueryAuthority` refuses the proposal (`NO_CANONICAL_ACTION`) rather than decide on invented facts ([ADR-0040](adr/0040-m3d-reconciliation-admission-across-tenures-and-undecidable-proposals.md)).
