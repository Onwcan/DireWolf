...
10. **As of M3d, the authority has durable state and nothing outside its process can reach it.** A DWKP message that decodes is well-formed and nothing more. The capability lattice (M3b) answers "is this within the authority held?", the policy engine (M3c) answers "should this be allowed?", and M3d gives both a durable subject: `kernel.db` holds fenced epochs, leases, admissions, grants and every policy input, and `audit.log` holds a hash chain written before any answer is returned ([ADR-0039](adr/0039-durable-authority-state.md)). But there is no kernel process serving requests, no transport and **no peer authentication** — M3d's callers assert their own identity, and only tests call it — no approval registry and no canonicaliser. So **no request from the runtime is authorised, no policy is enforced at any real boundary, no secret is protected and there is no process isolation** (M3e–M5). Policy inputs whose real producers do not exist yet are derived in the restrictive direction — every run is unattended (`api`), taint starts at none and can only rise — and are not end-to-end proven. And no proposed action is decided over DWKP before M4: without a canonicaliser the kernel cannot know where an action would run or what it would reach, so `QueryAuthority` refuses the proposal (`NO_CANONICAL_ACTION`) rather than decide on invented facts ([ADR-0040](adr/0040-m3d-reconciliation-admission-across-tenures-and-undecidable-proposals.md)).
