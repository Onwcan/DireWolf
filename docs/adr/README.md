...
**Accepted ADRs are immutable.** A changed decision produces a *new* ADR that supersedes or amends the old one; the old one stays, banner-marked, as a historical record. Phase 0.1 reconciliation produced twelve such ADRs (0018–0029) after adversarial review invalidated parts of the original set; M1 added 0030 (licence) and 0031 (repository layout and boundary enforcement); M2 added 0032 (the wire contract) and 0033 (protocol source of truth and TCB dependencies). **Do not cite a superseded ADR as current rationale.**
...
| **[0032](0032-wire-contract-framing-strict-json-and-jcs.md)** | **The wire contract: framing, strict JSON, and RFC 8785 as the one canonical encoding** | amends 0021, 0022, 0023; refines 0016 |
| **[0033](0033-protocol-source-of-truth-and-tcb-dependencies.md)** | **Rust → JSON Schema → Python; the dependencies `dwk-proto` brings into the TCB** | amends 0019; refines 0031 |
...
flowchart TD
  ...
  class A0,A18,A19,A20,A21,A22,A23,A24,A25,A26,A27,A28,A29,A30,A31,A32,A33 cur
  class A2s,A7s sup
```
...
1. ADRs are **immutable once accepted**. Supersede or amend; never edit history. A superseding ADR must say what survived from the old one, not only what changed.
...
3. Adding anything to `dwkd-authority`'s **transitive** dependency closure — which from M2 includes `dwk-proto`'s — requires a note on [ADR-0019](0019-language-rationale-v2.md) (or a record it links, as [ADR-0033](0033-protocol-source-of-truth-and-tcb-dependencies.md) is) and an entry in `architecture.toml` — that dependency set is a load-bearing claim, and `dwcheck deps` enforces the whole closure, not only the direct dependencies.
