...
*(As built at M4d, [ADR-0045](adr/0045-m4d-process-execution-broker.md) §§6, 16, 17: `process.exec` and `process.kill` are `NON_RETRYABLE` and `process.status` is `RETRY_SAFE`. A launch or a kill whose outcome is not proved — the broker lost after accepting it, a handshake that confirmed the helper but not the target, a signal whose delivery is uncertain — is recorded `UNKNOWN` and never performed again, live or after a restart; a launch refused before the target ran is `FAILED`.)*
