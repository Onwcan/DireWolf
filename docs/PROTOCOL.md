...
The example is illustrative: a real `direwolf.tool.invoke` (defined from M4b; §2) carries no `idempotency_key` and its payload is one typed call, `{"fs_read": {"path": "/workspace/…", "max_bytes": 4096}}`. Field formats, as decoded by `dwk-proto` ([`schemas/common/envelope.v1.schema.json`](../schemas/common/envelope.v1.schema.json)):
