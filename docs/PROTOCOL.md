...
The example is illustrative: a real version-1 `direwolf.tool.invoke` (defined from M4b; §2) carries no `idempotency_key` and its payload is one typed call, `{"fs_read": {"path": "/workspace/…", "max_bytes": 4096}}`; a version-2 one (M4c) carries a mandatory `idempotency_key` and exactly one of eight typed calls. Field formats, as decoded by `dwk-proto` ([`schemas/common/envelope.v1.schema.json`](../schemas/common/envelope.v1.schema.json)):
...
- **Defined by M4b:** `ToolInvoke` → `ToolResult` | `ToolDenial` | `ToolRefusal` | `ToolFailure`, and `CanonicalPreview` → `CanonicalPreviewResult` | `ToolRefusal` ([ADR-0043](adr/0043-m4b-private-broker-channel-and-brokered-fs-read.md)). Each request is one typed call — `fs_read{path, max_bytes}`, the only tool — with no tool name or argument map, no `cap_id` and no environment: the authority canonicalises the path, derives the required capability, chooses the environment and finds the covering grant. `ToolInvoke` has no idempotency key and is not replayed; a retry is a new invocation. Results carry the content as lowercase hexadecimal, at most 262 144 bytes, proven to fit one frame. `CanonicalPreview` decides exactly as `ToolInvoke` would and performs nothing; its answer authorises nothing. The authority → broker hop behind `ToolInvoke` is a separate, private protocol (`dwk_proto::brokerp`) that is not DWKP and has no schema here.
- **Version 2 by M4c:** the same operations and schemas, `schema_version` 2 ([ADR-0044](adr/0044-m4c-filesystem-operations-plans-and-atomic-mutation.md)). A request is exactly one of eight typed members — `fs_read`, `fs_list`, `fs_search`, `fs_stat`, `fs_write`, `fs_patch`, `fs_move`, `fs_delete` — and a `ToolInvoke` carries a mandatory `idempotency_key`, bound to exactly one invocation for ever (a reuse is `IDEMPOTENCY_KEY_REUSED`); a `CanonicalPreview` carries none. Every answer is a **plan**: each canonical action the call requires, with its role, verb, canonical path, whether an object is there, its byte count and both gates' decision; `ALLOW` only if every action is allowed. The reason vocabularies are new enumerations (`OBLIGATION_UNENFORCEABLE`, `MULTIPLY_LINKED`, `DESTINATION_EXISTS`, `PATCH_TOO_LARGE`, `WRITE_DENIED`, `SHARED_DIRECTORY`, `OUTCOME_UNKNOWN`, …), so version 1's still mean what they meant. Content travels inline and every bound is derived from the 1 MiB frame, which is unchanged: a write carries at most 256 KiB, and a patch's edits insert at most 256 KiB **together** — its largest request, envelope included, is proved to encode within 540 672 bytes. Version 1 is unchanged, and a request is answered in its own version.
- **Version 3 by M4d:** the same operations and schemas, `schema_version` 3 ([ADR-0045](adr/0045-m4d-process-execution-broker.md) §4). A request is exactly one of eleven typed members — the eight of version 2 and `process_exec{executable, args, cwd?}`, `process_status{process_id}`, `process_kill{process_id}`; a process is named by the opaque id its launch returned, never a pid, and the request carries no `argv[0]`, digest, environment or classification — the authority derives them. Versions 1 and 2 are unchanged.
- **Reserved:** every other operation below. A reserved operation has no message name, no schema and no decoder; a message naming one is `PROTOCOL_UNKNOWN_OPERATION`. `QueryInvocationStatus` ([RELIABILITY.md](RELIABILITY.md)) is reserved too. `ToolInvoke` stayed reserved through M3 because its request had to name a tool with arguments the kernel canonicalises, and neither existed before M4; the alternatives were an opaque argument map or a duplicate of `QueryAuthority`, and both failed the protocol change review.
